...
- **実在するのに双方向とも到達できない住所があります。** 隣接エントリの読みを持ってしまっている
  行は信頼できないためです。札幌市中央区の `円山` が典型で、カナもローマ字も `円山西町` のものに
  なっています。`toRomaji` は `CORRUPT_ROMAJI_DATA` を返し、`fromRomaji` も候補に出しません。
- **町字名の短縮形はしばしば曖昧です。** 全国で約880件、接尾辞を落とした形が同一市区町村内の
  別の町字のフルネームと一致します（函館市には `昭和町` と `昭和一丁目` の両方が存在するため
  `"Showa"` は両方に一致）。フルネーム（`"Showa-cho"`）を書けば解決し、本ライブラリの出力は
  常にフルネームです。短縮形を手入力した場合は候補付きの `AMBIGUOUS` が返ります。
