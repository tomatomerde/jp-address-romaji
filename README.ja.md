...
- **町字名の短縮形は曖昧になり得ます。** 出荷データセットに対してマッチャ自身のキー関数で
  実測（`scripts/measure-ambiguity.ts`）: `fromRomaji` が索引するローマ字キーの 1.07%
...
97.99% は市区町村が判明していれば一意です（`scripts/measure-ambiguity.ts` による出荷データセット
での実測）が、全国文脈では大きく落ち、市区町村名も全国で 13 件衝突します
...
（211,041 中 1,406、関与する町字は全国 2,622。`scripts/measure-ambiguity.ts` による実測）に
