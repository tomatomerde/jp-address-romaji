 ...
 * (docs/coverage.md, generated by scripts/measure-coverage.ts), a town's
 * full-form romanization key is unique within a known municipality 97.95% of
 * the time but only 59.92% of the time nationally, and 39 groups of
 * municipalities in different prefectures share a romanization, 86
 * municipalities in all ("Date-shi" is both 北海道伊達市 and 福島県伊達市).
 *
 ...
 *
 * Every figure in this file is quoted from that generated report;
 * scripts/check-quoted-figures.ts fails the build if one drifts from it.
 */
...
export interface FromRomajiOptions {
   ...
   * short forms are included) — and roughly half of the indexed collisions
   * disappear when the caller writes the full town name. Bundling Japan Post's KEN_ALL to close
 ...
 *
 * Exported (along with {@link candidateKeys} and {@link exactKeys}) for
 * scripts/lib/ambiguity.ts, so the published ambiguity figures are measured
 * with the exact functions the matcher runs — not a transcription that can
 * drift. Not part of the public API: index.ts does not re-export it.
 */
