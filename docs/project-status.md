...
最新は `0.1.4`（2026-08-14）。`0.1.3` は 2026-08-13、`0.1.0` は 2026-08-10、`0.1.1`・`0.1.2` は
...
**`0.1.5` の作業がこのブランチで進行中——まだリリースしていない。** `0.1.4` が入れた小字
（koaza）の扱いに2つの問題が残っていたのを直すもの: (1) 読みが単位（`３チョウメ`）で止まって
いるのに名前が続く（`三丁目大横`）形が完全性チェックを素通りし、`大横` を落としたまま `ok` を
返していた——`0.1.4` が直したと主張している当の住所がこれ。(2) 上流が選んだ行の小字を無条件に
出力へ入れていたため、**入力に書かれていない小字が宛名に現れていた**（`大字小野1-1` →
`"1-1 Azamachi Ono"`。実データ300件のサンプルで8件）。詳細は CHANGELOG の `0.1.5` の項。
`package.json` は `0.1.5` に上げてあるが、publish はまだ——実行したらこの段落を書き換えること。
