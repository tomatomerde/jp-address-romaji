...
- **`fromRomaji` にも「町名の先頭だけに一致し、余りを建物名にする」経路が残っている。**
  ...
  **区別できないことは測ってある**（2026-08-19、デモが配信する9市区町村から501町を抽出。
  土台は `toRomaji` が `ok` を返し、かつ往復できたものだけ）。同じ位置——町名の直後、
  最初のカンマの手前——に語を1つ足して `fromRomaji` に渡した結果:

  | 足した語 | 現在の答え | あるべき答え |
  | --- | --- | --- |
  | 建物名らしい語（`Sunshine Bldg 5F`・`Sakura Building`） | 1,002件中1,002件が `ok` | `ok`（正しい） |
  | 町名らしい語（`9-Jo`・`Nishi`） | 1,002件中1,002件が `ok`、うち999件が元の町を返す | 失敗（全部誤答） |

  **両者は100%同じ答えになる。** 順方向では「空白で区切られているか」という呼び出し側の
  句読点が使えたが、逆方向ではその空白が単語の区切りそのものなので、手がかりとして残らない。
  語彙で判定する案（「町名に現れる語なら住所」）も、`Sakura` のように**建物名にも町名にも
  現れる語**があるため、そのままでは同じ表に落ちる。**規則を作るなら別の根拠が要る**——
  この測定は「まだ根拠が無い」ことの裏づけであって、直せないことの証明ではない。
