...
const KUROSHIRO = path.join(ROOT, 'docs', 'measurements', 'kuroshiro.json');
...
/**
 * `docs/measurements/kuroshiro.json`, written by `scripts/compare-kuroshiro.mjs`.
 *
 * Unlike the coverage figures this one is not regenerated by any workflow: it
 * needs the network and an analyzer that is not a dependency here, so it is
 * run by hand. That makes pinning the sentences more important rather than
 * less — nothing else will notice when a re-measurement moves a number.
 */
interface Kuroshiro {
  measuredAt: string;
  kuroshiroVersion: string;
  overall: { towns: number; differ: number; pct: string };
  excludingNotation: { towns: number; differ: number; pct: string };
  ...
  examples: ReadonlyArray<{ town: string; official: string; kuroshiro: string; differs: boolean }>;
}
...
interface Expectation {
  ...
  what: string;
  /** The exact text the file must contain, built from the measurements. */
  text: (f: Figures, k: Kuroshiro) => string;
}
...
const EXPECTATIONS: Expectation[] = [
  ...
  // --- the kuroshiro comparison, in both READMEs ---
  //
  // The paragraph these figures live in is the first thing a reader who is
  // deciding between this package and an analyzer reads, so a stale number
  // there is worse than a stale number in the coverage table.
  {
    file: 'README.md',
    what: 'how often kuroshiro disagrees with the official romanization',
    text: (_f, k) =>
      `**${n(k.overall.differ)} of ${n(k.overall.towns)} town names (${k.overall.pct}%)**`,
  },
  {
    file: 'README.md',
    what: 'the per-municipality breakdown of that comparison',
    text: (_f, k) => municipalityTable(k, (m) => `| ${m.prefecture} ${m.municipality} | ${n(m.towns)} | ${m.pct}% |`) +
      `| **All nine** | **${n(k.overall.towns)}** | **${k.overall.pct}%** |`,
  },
  {
    file: 'README.md',
    what: 'the same comparison with notation-only mismatches excluded',
    text: (_f, k) =>
      `leaves ${n(k.excludingNotation.differ)} of ${n(k.excludingNotation.towns)} ` +
      `(${k.excludingNotation.pct}%)`,
  },
  {
    file: 'README.ja.md',
    what: 'how often kuroshiro disagrees with the official romanization',
    text: (_f, k) =>
      `**${n(k.overall.towns)} 件中 ${n(k.overall.differ)} 件（${k.overall.pct}%）で食い違う**`,
  },
  {
    file: 'README.ja.md',
    what: 'the per-municipality breakdown of that comparison',
    text: (_f, k) => municipalityTable(k, (m) => `| ${m.prefecture} ${m.municipality} | ${n(m.towns)} | ${m.pct}% |`) +
      `| **9市区町村 合計** | **${n(k.overall.towns)}** | **${k.overall.pct}%** |`,
  },
  {
    file: 'README.ja.md',
    what: 'the same comparison with notation-only mismatches excluded',
    text: (_f, k) =>
      `${n(k.excludingNotation.towns)} 件中 ${n(k.excludingNotation.differ)} 件` +
      `（${k.excludingNotation.pct}%）が残る`,
  },

  // The four names that carry the argument, and the four misreadings beside
  // them. Pinning only the percentage would leave the sentence that persuades
  // anyone — 中町 is Nakacho but 細工町 is Saikumachi — checked by nothing,
  // and it is a claim about a shipping dataset that can change under it.
  {
    file: 'README.md',
    what: 'the cho/machi split inside one ward',
    text: (_f, k) =>
      `中町 is \`${official(k, '中町')}\` but 細工町 is \`${official(k, '細工町')}\`; ` +
      `四谷坂町 is \`${official(k, '四谷坂町')}\` but 四谷本塩町 is \`${official(k, '四谷本塩町')}\``,
  },
  {
    file: 'README.md',
    what: 'the four plain misreadings',
    text: (_f, k) => misreadings(k, (e) => `${e.town} \`${e.official}\` → \`${e.kuroshiro}\``, ', '),
  },
  {
    file: 'README.ja.md',
    what: 'the cho/machi split inside one ward',
    text: (_f, k) =>
      `\`中町\` は \`${official(k, '中町')}\`、\`細工町\` は \`${official(k, '細工町')}\`。` +
      `\`四谷坂町\` は \`${official(k, '四谷坂町')}\`、` +
      `\`四谷本塩町\` は \`${official(k, '四谷本塩町')}\`。`,
  },
  {
    file: 'README.ja.md',
    what: 'the four plain misreadings',
    text: (_f, k) =>
      misreadings(k, (e) => `\`${e.town}\` \`${e.official}\` → \`${e.kuroshiro}\``, '、'),
  },
];

/** The official romanization the measurement recorded for one quoted town. */
function official(k: Kuroshiro, town: string): string {
  const found = k.examples.find((e) => e.town === town);
  if (!found) throw new Error(`docs/measurements/kuroshiro.json has no example for ${town}`);
  return found.official;
}

/**
 * The misreading list, in the order both READMEs print it.
 *
 * Only these four: the other quoted names differ over cho/machi, which is the
 * separate point made just above them, and mixing the two would let a rewrite
 * of one sentence be satisfied by the other.
 */
const MISREAD = ['白銀町', '須賀町', '横寺町', '原町'];

function misreadings(
  k: Kuroshiro,
  row: (e: Kuroshiro['examples'][number]) => string,
  separator: string,
): string {
  return MISREAD.map((town) => {
    const found = k.examples.find((e) => e.town === town);
    if (!found) throw new Error(`docs/measurements/kuroshiro.json has no example for ${town}`);
    if (!found.differs) {
      throw new Error(`${town} no longer differs from the official romanization; the READMEs still say it does`);
    }
    return row(found);
  }).join(separator);
}
...
/**
 * The comparison table, rows in the order the READMEs print them.
 *
 * Both files sort by how often the analyzer differs, ascending — a reader
 * should meet the best case first and see the spread, not just the worst row.
 * `compare-kuroshiro.mjs` emits them in demo/municipalities.txt order instead,
 * so the sort belongs here rather than in the measurement.
 */
function municipalityTable(k: Kuroshiro, row: (m: Kuroshiro['municipalities'][number]) => string): string {
  return [...k.municipalities]
...
function main(): void {
  ...
  if (!fs.existsSync(KUROSHIRO)) {
    console.error(
      `::error::${path.relative(ROOT, KUROSHIRO)} is missing. Regenerate it by hand: ` +
        'npm install --no-save kuroshiro kuroshiro-analyzer-kuromoji && ' +
        'node scripts/compare-kuroshiro.mjs > docs/measurements/kuroshiro.json',
    );
    process.exit(1);
  }
  const figures = JSON.parse(fs.readFileSync(FIGURES, 'utf8')) as Figures;
  const kuroshiro = JSON.parse(fs.readFileSync(KUROSHIRO, 'utf8')) as Kuroshiro;
  ...
  for (const expectation of EXPECTATIONS) {
    ...
    // The helpers below throw when the measurement stops containing what an
    // expectation reads out of it — a quoted town vanishing from the example
    // list, or one of them agreeing with the analyzer after a data refresh.
    // Report that the same way as a stale figure instead of letting a stack
    // trace stand in for the explanation.
    let text: string;
    try {
      text = expectation.text(figures, kuroshiro);
    } catch (error) {
      failures++;
      console.error(
        `::error file=${expectation.file}::the measurement no longer supports what ${expectation.file} ` +
          `says about ${expectation.what}: ${error instanceof Error ? error.message : String(error)}`,
      );
      continue;
    }
    ...
    console.error(
      `::error file=${expectation.file}::${expectation.file} no longer states the measured figure ` +
        `for ${expectation.what}. The last measurement says it should read:\n` +
        `${text}\n` +
  ...
  if (failures > 0) {
    console.error(
      `\n${failures} quoted figure(s) are out of date. The measurement is the source: ` +
        'everything under docs/measurements/ is generated, the prose is not.',
    );
    process.exit(1);
  }
  console.log(
    `All ${EXPECTATIONS.length} quoted figures match their measurements ` +
      `(coverage ${figures.measuredAt}, kuroshiro ${kuroshiro.measuredAt}).`,
  );
}
