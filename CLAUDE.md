<!-- テンプレート原本: dev-standards/CLAUDE.template.md @ 758d2d8。共通部分（「ここから下は共通」以降）を修正したら原本にも反映すること -->
...
**このファイルは公開リポジトリに置かれる前提で書くこと。** 読み手は次のセッションだけではなく、
リポジトリを訪れた第三者でもある。セッション間の申し送りであっても、外部の読者が読んで意味が
通り、かつ不快でない書き方にする。具体的には、特定の人物や依頼のやりとりについての内輪の評価を
書かない。技術的な事実・未検証事項・残作業は、遠慮せずそのまま書いてよい。
